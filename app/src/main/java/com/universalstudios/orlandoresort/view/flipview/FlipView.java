package com.universalstudios.orlandoresort.view.flipview;
...
import com.universalstudios.orlandoresort.view.flipview.Recycler.Scrap;
...
public class FlipView extends FrameLayout {
	...
	private static final int MAX_SHADOW_ALPHA = 60;// out of 255
	private static final int MAX_SHADE_ALPHA = 70;// out of 255
	private static final int MAX_SHINE_ALPHA = 70;// out of 255
	...
	private float mShadowPaddingPx = 0.0f;
	...
	public FlipView(Context context, AttributeSet attrs, int defStyle) {
		...
		mIsCircular = a.getInt(R.styleable.FlipView_shape,
				SHAPE_RECTANGULAR) == SHAPE_CIRCULAR;

		mShadowPaddingPx = a.getDimension(R.styleable.FlipView_shadowPadding, 0.0f);
	...
	private void setFlipDistance(float flipDistance) {
		...
		if (mCurrentPageIndex != currentPageIndex) {
			...
			// be smarter about this. Dont remove a view that will be added
				...
				if (mIsFlipping) {
					...
					final boolean isOverFlipping = mFlipDistance < minFlipDistance
							...
							if (isOverFlipping) {
								...
								if (mOnOverFlipListener != null) {
									//  in the future should only notify flip distance 0
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mIsFlipping || !mScroller.isFinished() || mPeakAnim != null) {
			...
			// not the prettiest to have this with the drawing logic,
			// should change.
	...
	private void drawPreviousShadow(Canvas canvas) {
		...
		if (degreesFlipped > 90) {
			...
			if (isCircular()) {
				canvas.drawCircle(canvas.getWidth() / 2.0f,
						canvas.getHeight() / 2.0f,
						(isFlippingVertically() ? canvas.getHeight() / 2.0f : canvas.getWidth() / 2.0f) - mShadowPaddingPx,
						mShadowPaint);
			}
			else {
	...
	private void drawFlippingShadeShine(Canvas canvas) {
		...
		if (degreesFlipped < 90) {
			...
			if (isCircular()) {
				float radius = (isFlippingVertically() ? Math.abs(mBottomRect.top - mBottomRect.bottom) :
					Math.abs(mRightRect.left - mRightRect.right))
					- mShadowPaddingPx;

				canvas.drawCircle(isFlippingVertically() ? mBottomRect.exactCenterX() : mRightRect.left,
						isFlippingVertically() ? mBottomRect.top : mRightRect.exactCenterY(),
								radius,
								mShinePaint);
			}
			else {
			...
			if (isCircular()) {
				float radius = (isFlippingVertically() ? Math.abs(mTopRect.top - mTopRect.bottom) :
					Math.abs(mLeftRect.left - mLeftRect.right))
					- mShadowPaddingPx;

				canvas.drawCircle(isFlippingVertically() ? mTopRect.exactCenterX() : mLeftRect.right,
						isFlippingVertically() ? mTopRect.bottom : mLeftRect.exactCenterY(),
								radius,
								mShadePaint);
			}
			else {
	...
	private void peak(boolean next, boolean once) {
		...
		mPeakAnim.setRepeatCount(once ? 3 : ValueAnimator.INFINITE);
		mPeakAnim.start();
	}
	 ...
	 *            view
	 */
	public void peekNext(boolean once) {
		if (mCurrentPageIndex < mPageCount - 1) {
			peak(true, once);
		}
	}
	 ...
	 *            view
	 */
	public void peekPrevious(boolean once) {
		if (mCurrentPageIndex > 0) {
			peak(false, once);
		}
	}
	...
	public void setOnOverFlipListener(OnOverFlipListener onOverFlipListener) {
		mOnOverFlipListener = onOverFlipListener;
	}
