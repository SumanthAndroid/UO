...
import com.universalstudios.orlandoresort.view.calendar.MonthCellDescriptor.RangeState;
...
public class CalendarPickerView extends ListView {
	...
	public CalendarPickerView(Context context, AttributeSet attrs) {
		...
		final int bg = a.getColor(R.styleable.CalendarPickerView_android_background,
				res.getColor(R.color.calendar_background));
		...
		dayBackgroundResId = a.getResourceId(R.styleable.CalendarPickerView_dayBackground,
				R.drawable.state_list_calendar_background_selector);
		dayTextColorResId = a.getResourceId(R.styleable.CalendarPickerView_dayTextColor,
				R.color.state_list_calendar_text_selector);
		titleTextColor = a.getColor(R.styleable.CalendarPickerView_calendarTitleTextColor,
				res.getColor(R.color.calendar_text_month_title));
		...
		locale = Locale.US;
		today = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
		minCal = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
		maxCal = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
		monthCounter = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
		monthNameFormat = new SimpleDateFormat(context.getString(R.string.month_name_format), locale);
		monthNameFormat.setTimeZone(DateTimeUtils.getParkTimeZone());
		weekdayNameFormat = new SimpleDateFormat(context.getString(R.string.day_name_format), locale);
		weekdayNameFormat.setTimeZone(DateTimeUtils.getParkTimeZone());
		fullDateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM, locale);
		fullDateFormat.setTimeZone(DateTimeUtils.getParkTimeZone());

		if (isInEditMode()) {
			Calendar nextYear = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
	...
	private void scrollToSelectedDates() {
		...
		Calendar today = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
	...
	private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
		Calendar newlySelectedCal = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
	...
	public void highlightDates(Collection<Date> dates) {
		for (Date date : dates) {
			...
			if (monthCellWithMonthIndex != null) {
				Calendar newlyHighlightedCal = Calendar.getInstance(DateTimeUtils.getParkTimeZone());
	...
	private MonthCellWithMonthIndex getMonthCellWithIndexByDate(Date date) {
		int index = 0;
		Calendar searchCal = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
		searchCal.setTime(date);
		Calendar actCal = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
	...
	List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
		Calendar cal = Calendar.getInstance(DateTimeUtils.getParkTimeZone(), locale);
	...
	private class DefaultOnInvalidDateSelectedListener implements OnInvalidDateSelectedListener {
		@Override public void onInvalidDateSelected(Date date) {
			...
			//		errMessage, Toast.LENGTH_SHORT, getContext());
