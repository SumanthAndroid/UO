 ...
 *
 */
public class OfferImagesAndDetailFragment extends DatabaseQueryFragment implements OnPageChangeListener,
        PicassoProvider {

    private static final String TAG = OfferImagesAndDetailFragment.class.getSimpleName();
    ...
    public static OfferImagesAndDetailFragment newInstance(DatabaseQuery databaseQuery) {
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "newInstance: databaseQuery = " + databaseQuery);
        }

        // Create a new fragment instance
        OfferImagesAndDetailFragment fragment = new OfferImagesAndDetailFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View fragmentView = inflater.inflate(R.layout.fragment_offer_detail_images_and_title, container,
                false);

        // Setup Views
        mViewPagerContainer = (RelativeLayout) fragmentView
                .findViewById(R.id.fragment_offer_images_and_title_viewpager_container);
        mViewPager = (JazzyViewPager) fragmentView
                .findViewById(R.id.fragment_offer_images_and_title_viewpager);
        mPagerDotContainer = (LinearLayout) fragmentView
                .findViewById(R.id.fragment_offer_images_and_title_dot_layout);
        mBottomGradient = fragmentView.findViewById(R.id.fragment_offer_images_and_title_bottom_gradient);
        mVenueNameText = (TextView) fragmentView
                .findViewById(R.id.fragment_offer_images_and_title_venue_name_text);
        mNameText = (TextView) fragmentView.findViewById(R.id.fragment_offer_images_and_title_poi_name_text);
        mDescText = (TextView) fragmentView.findViewById(R.id.fragment_offer_images_and_title_poi_desc_text);
        ...
        switch (loader.getId()) {
            case LOADER_ID_DATABASE_QUERY:
                ...
                if (data != null && data.moveToFirst()) {
                    ...
                    String venueName = data
                            .getString(data.getColumnIndex(VenuesTable.COL_ALIAS_DISPLAY_NAME));

                    Offer offer = GsonObject.fromJson(offerObjectJson, Offer.class);
                    String desc = TextUtils.isEmpty(offer.getLongDescription()) ? offer.getShortDescription()
                            : offer.getLongDescription();
                    String name = offer.getDisplayName();

                    mVenueNameText.setText(venueName != null ? venueName.toUpperCase(Locale.US) : "");
                    mVenueNameText.setVisibility(TextUtils.isEmpty(venueName) ? View.GONE : View.VISIBLE);
                    ...
                    if (!mHasLoadedData) {
                        ...
                        List<String> imageUrls = offer.getDetailImageUrls();
                        if (imageUrls != null) {
